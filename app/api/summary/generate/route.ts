...
export async function POST(request: Request) {
  try {
    ...
    try {
      ...
      const raw = error instanceof Error ? error.message : "Summary generation failed";
      // Missing key on Preview is a common cause — make it obvious.
      if (/ANTHROPIC_API_KEY is not set/i.test(raw)) {
        return Response.json(
          { error: "AI summaries aren't configured on this deployment (missing ANTHROPIC_API_KEY)." },
          { status: 503 },
        );
      }
